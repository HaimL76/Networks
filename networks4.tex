...
\end{cases}\]
But $r\geq{0}\implies-\alpha{j}+\beta(1-r-j)j\leq-\alpha{j}+\beta(1-j)j$, so when recovery appears in the system, the growth of infected in time for SIR is less or equal to that of SIS, but while $r=0$, the growth of infected is identical.
\end{document}
