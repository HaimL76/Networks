...
\section{}
\subsection{}
We have a potential edge between node $0$ and each one of the other $N$ nodes. So for node $0$ to have all potential edges, the probability is $e^{-\alpha}e^{-2\alpha}\cdots{e^{-N\alpha}}$

For node $1$, we already have the edge $\{0,1\}$, so the probability to have all the edges is $e^{-\alpha}e^{-2\alpha}\cdots{e^{-(N-1)}\alpha}$

For node $2$, we already have nodes $\{0,2\},\{1,2\}$, so the probability to have all the edges is $e^{\alpha}e^{-2\alpha}\cdots{e^{-(N-2)\alpha}}$ and so on, until node $N$ who already has all its edges with nodes $\{0,1,\dots,N-1\}$, so it does not change the probability.

We conclude that the probability to have all the possible edges in the network is $e^{-(N-1)\alpha}e^{-(N-2)2\alpha}\cdots{e^{-1\cdot{(N-1)}\alpha}}=\prod_{i=1}^Ne^{-(N-i)i\alpha}$, and the degree in this case is $d=N-1$.


Now we omit one edge from one of the nodes.
