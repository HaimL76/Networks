...
\[
E(k^n)=\int_1^{\infty}k^nCk^{-\gamma}dk=\int_1^{\infty}Ck^{n-\gamma}dk=\frac{C}{n-\gamma+1}k^{n-\gamma+1}\bigg\rvert_1^{\infty}
\]
...
\subsection{}
The mean is $E(k)=E(k^1)$, so it diverges for $\gamma\leq{1+1}=2$ and converges for $\gamma>2$.

The variance is $E(k^2)$, so it diverges for $\gamma\leq{2+1}=3$ and converges for $\gamma>3$.
\end{document}
